    ...
    .map((act) => {
        ...
        const imagenHtml = act.imagen
            ? `<div style="margin-top:6px;">
                    <img src="${escapeAttr(act.imagen)}" 
                         alt="${escapeAttr(act.titulo)}"
...
`;
    ...
    document.querySelectorAll(".borrar-estudiante").forEach((btn) => {
        btn.addEventListener("click", async (e) => {
            ...
            if (!confirm("¿Eliminar este estudiante? Esta acción también eliminará sus actividades (si la FK es cascade)."))
                return;
